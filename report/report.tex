...
\paragraph{Soluzione} Ho creato un'interfaccia generica che supporta qualsiasi tipo di trippletta di valori, poi implementata in una classe generica.
Per poter sommare i valori ho poi creato una classe con metodi statici che fa la somma delle tripplette.
...
\begin{itemize}
	...
	\item Tripplette di valori (package bbblast.utils Triplet, TripletImpl )
	\item Somma di Tripplette di valori interi (package bbblast.utils TripletIntegerUtility)
\end{itemize}
...
Sono soddisfatta del mio lavoro individuale, ma non del progetto in totale. Ci sono stati parecchi problemi sulle parti del codice in comune, perchè spesso chi doveva fare una parte non parlava o non si capiva
con chi avrebbe fatto il resto dell'implementazione. Questo mi ha rallentato nello sviluppo della mia parte e a tratti ha costretto a riscrivere parti di codice che non rispettavano l'idea altrui. Non ha aiutato 
la coordinazione del progetto anche il fatto che essendo al nostro primo progetto, alcuni aspetti che inizialmente ritenevamo dettagli si sono rivelati problemi implementativi ben maggiori. 
In futuro cercherò di spiegarmi meglio sulle caratteristiche del software prodotto da me e comunicare meglio con i miei compagni di progetto cosa aspettarmi dal loro codice. 
